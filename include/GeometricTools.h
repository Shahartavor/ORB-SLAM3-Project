...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        static bool Triangulate(Eigen::Vector3f &x_c1, Eigen::Vector3f &x_c2,
                                ...
                                Eigen::Matrix3f &mK_1, Eigen::Matrix3f &mK_2,
                                Eigen::Matrix<float, 3, 3> &Rcw1, Eigen::Matrix<float, 3, 3> &Rcw2,
                                Eigen::Vector3f &tcw1,
...
//Eigen::Matrix<float,3,4> &Tc1w ,Eigen::Matrix<float,3,4> &Tc2w,Eigen::Vector3f &x3D);
        ...
        static bool
        CheckMatrices(const Eigen::Matrix<T, rows, cols> &eigMat1, const Eigen::Matrix<T, rows, cols> &eigMat2) {
            const float epsilon = 1e-3;
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
